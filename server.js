...
app.post('/api/quote/:id/reserve', async (req, res) => {
  try {
    ...
    if (!isAch && guestId) {
      try {
        const attachRes = await fetch(`https://open-api.guesty.com/v1/guests/${guestId}/payment-methods`, {
          method:  'POST',
          headers: { Authorization: `Bearer ${openToken}`, 'Content-Type': 'application/json' },
          body:    JSON.stringify({
        ...
        console.log('Guesty card attach:', attachRes.status, (await attachRes.text()).slice(0, 200));
